...
namespace Unity_Technologies.Recorder.Framework.Core.Engine
{
    ...
    public struct FileNameGenerator
    {
        ...
        public string BuildFileName( RecordingSession session, int frame, int width, int height, string ext )
        {
            ...
            if (match.Success)
            {
                this.m_FramePattern = match.Value;
                this.m_FramePatternDst = this.m_FramePattern.Substring(1,this.m_FramePattern.Length-2 );
            }
            else
            {
                this.m_FramePattern = "<0>";
                this.m_FramePatternDst = "0";
            }             

            var fileName  = this.pattern.Replace(tags[(int)ETags.Extension], ext)
                .Replace(tags[(int)ETags.Resolution], string.Format("{0}x{1}", width, height))
                .Replace(this.m_FramePattern, frame.ToString(this.m_FramePatternDst))
