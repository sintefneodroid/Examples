...
namespace Common.Scripts {
  ...
  /// <summary>
  ///   A thread-safe class which holds a queue with actions to execute on the next Update() method. It can be used
  ///   to make calls to the main thread for
  ...
  public class UnityMainThreadDispatcher : MonoBehaviour {
    static readonly Queue<Action> _execution_queue = new Queue<Action>();
    ...
    /// </summary>
    public void Update() {
      lock (_execution_queue) {
        while (_execution_queue.Count > 0) {
          _execution_queue.Dequeue().Invoke();
        }
      }
    }
