...
// Since this script is used for visual debugging, we exclude most of it in builds.
    ...
    void DrawCharactersBounds() {
      ...
      for (var i = 0; i < textInfo.characterCount; i++) {
        ...
        var bottomRight =
            this.m_Transform.TransformPoint(new Vector3(cInfo.bottomRight.x, cInfo.bottomRight.y, 0));

        var color = cInfo.isVisible ? Color.yellow : Color.grey;
        this.DrawRectangle(bottomLeft, topLeft, topRight, bottomRight, color);

        // Baseline
        var baselineStart = new Vector3(
            topLeft.x,
            this.m_Transform.TransformPoint(new Vector3(0, cInfo.baseLine, 0)).y,
            0);
        var baselineEnd = new Vector3(
            topRight.x,
            this.m_Transform.TransformPoint(new Vector3(0, cInfo.baseLine, 0)).y,
            0);
        ...
        var ascenderStart = new Vector3(
            topLeft.x,
            this.m_Transform.TransformPoint(new Vector3(0, cInfo.ascender, 0)).y,
            0);
        var ascenderEnd = new Vector3(
            topRight.x,
            this.m_Transform.TransformPoint(new Vector3(0, cInfo.ascender, 0)).y,
            0);
        var descenderStart = new Vector3(
            bottomLeft.x,
            this.m_Transform.TransformPoint(new Vector3(0, cInfo.descender, 0)).y,
            0);
        var descenderEnd = new Vector3(
            bottomRight.x,
            this.m_Transform.TransformPoint(new Vector3(0, cInfo.descender, 0)).y,
            0);
        ...
        var capHeightStart = new Vector3(
            topLeft.x,
            this.m_Transform.TransformPoint(new Vector3(0, capHeight, 0)).y,
            0);
        var capHeightEnd = new Vector3(
            topRight.x,
            this.m_Transform.TransformPoint(new Vector3(0, capHeight, 0)).y,
            0);
        ...
        for (var j = 0; j < wInfo.characterCount; j++) {
          ...
          if (isBeginRegion == false && isCharacterVisible) {
            ...
            if (wInfo.characterCount == 1) {
              ...
              bottomRight =
                  this.m_Transform.TransformPoint(new Vector3(currentCharInfo.topRight.x, minDescender, 0));
              topRight = this.m_Transform.TransformPoint(
    ...
    void DrawLineBounds() {
      ...
      for (var i = 0; i < textInfo.lineCount; i++) {
        ...
        var bottomOrigin = this.m_Transform.TransformPoint(
            new Vector3(textInfo.characterInfo[lineInfo.firstCharacterIndex].origin, descender, 0));
        var topOrigin = this.m_Transform.TransformPoint(
            new Vector3(textInfo.characterInfo[lineInfo.firstCharacterIndex].origin, ascender, 0));
        var bottomAdvance = this.m_Transform.TransformPoint(
            new Vector3(
                textInfo.characterInfo[lineInfo.firstCharacterIndex].origin + maxAdvance,
                descender,
                0));
        var topAdvance = this.m_Transform.TransformPoint(
            new Vector3(
                textInfo.characterInfo[lineInfo.firstCharacterIndex].origin + maxAdvance,
                ascender,
                0));
        ...
        var baselineStart = this.m_Transform.TransformPoint(
            new Vector3(textInfo.characterInfo[lineInfo.firstCharacterIndex].bottomLeft.x, baseline, 0));
        var baselineEnd = this.m_Transform.TransformPoint(
