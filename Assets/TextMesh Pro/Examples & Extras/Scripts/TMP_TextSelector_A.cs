    ...
    void LateUpdate() {
      ...
      if (this.m_isHoveringObject) {
        ...
        if (wordIndex != -1 && wordIndex != this.m_lastWordIndex) {
          ...
          var wordPOS = this.m_TextMeshPro.transform.TransformPoint(
              this.m_TextMeshPro.textInfo.characterInfo[wInfo.firstCharacterIndex].bottomLeft);
          ...
          for (var i = 0; i < wInfo.characterCount; i++) {
            var vertexIndex = this.m_TextMeshPro.textInfo.characterInfo[wInfo.firstCharacterIndex + i]
                .vertexIndex;
